...
const mergeContactInformation = (
    ...
    for (const key in filteredContactInformationFromApi) {
        ...
        const keepCompanyTaxNumberFromApi = key === 'companyTaxNumber' && isEmptyString;

        if (keepCompanyTaxNumberFromApi) {
            continue;
        }
    ...
    if (contactInformationFromStore.deliveryAddressUuid === 'new-delivery-address') {
        filteredContactInformationFromApi.deliveryAddressUuid = contactInformationFromStore.deliveryAddressUuid;
    }
