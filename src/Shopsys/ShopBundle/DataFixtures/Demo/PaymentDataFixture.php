...
class PaymentDataFixture extends AbstractReferenceFixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        foreach ($this->domain->getAllIncludingDomainConfigsWithoutDataCreated() as $domain) {
            ...
            $paymentData->pricesByCurrencyId[$currency->getId()] = Money::create((string)(99.95 / $currency->getExchangeRate()));
        }
        ...
        foreach ($this->domain->getAllIncludingDomainConfigsWithoutDataCreated() as $domain) {
            $currency = $this->currencyFacade->getDomainDefaultCurrencyByDomainId($domain->getId());
            $paymentData->pricesByCurrencyId[$currency->getId()] = Money::create((string)(49.90 / $currency->getExchangeRate()));
        }
        ...
        foreach ($this->domain->getAllIncludingDomainConfigsWithoutDataCreated() as $domain) {
            $currency = $this->currencyFacade->getDomainDefaultCurrencyByDomainId($domain->getId());
            $paymentData->pricesByCurrencyId[$currency->getId()] = Money::zero();
        }
